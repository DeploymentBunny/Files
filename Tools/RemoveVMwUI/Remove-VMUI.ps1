<#
.SYNOPSIS
    GUI tool to list and permanently remove virtual machines on the local Hyper-V host.

.DESCRIPTION
    Remove-VMUI provides a Windows Forms interface for Hyper-V VM cleanup on the local host.
    - Starts without requiring elevation at launch.
    - Shows an "Elevate (Admin)" button when running as a standard user.
      Clicking the button relaunches the UI as Administrator via UAC.
    - Lists VMs on the local Hyper-V host and permanently deletes selected VMs including
      checkpoints/snapshots, attached virtual disk files, and the configuration folder.
    - Verbose mode can be toggled via checkbox; log filename is shown in the output pane.
    - Displays operation output in a scrollable output pane.
    - Confirms destructive deletions before proceeding.
    - Writes a timestamped log file to %TEMP%\Remove-VMUI\.
    - Persists last-used settings (Verbose state) to
      %LOCALAPPDATA%\DeploymentBunny\Remove-VMUI.settings.json.

.EXAMPLE
    .\Remove-VMUI.ps1

    Launches the UI, connects to the local host by default, and allows interactive VM removal.

.NOTES
    FileName:    Remove-VMUI.ps1
    Author:      Mikael Nystrom
    Contact:     deploymentbunny@outlook.com
    Created:     2017-01-01
    Updated:     2026-04-28
    Twitter:     @mikael_nystrom

    Disclaimer:
    This script is provided "AS IS" with no warranties, confers no rights and
    is not supported by the author.
.LINK
    https://www.deploymentbunny.com
.FUNCTIONALITY
    Starts directly in Windows Forms UI without prompting at startup.
    If running as a standard user, an "Elevate (Admin)" button is shown in the form.
    Clicking it relaunches the script elevated via Start-Process -Verb RunAs and closes
    the current non-elevated UI instance.
    Requires local Administrator privileges to manage local Hyper-V VMs.
    On form load, last-used Verbose checkbox state is restored from the
    settings file. On form close, current value is saved back.
#>

$Host.UI.RawUI.WindowTitle = $myInvocation.MyCommand.Definition

Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
[System.Windows.Forms.Application]::EnableVisualStyles()

$Font1        = [System.Drawing.Font]::new("Arial", 10, [System.Drawing.FontStyle]::Bold)
$FontHeading1 = [System.Drawing.Font]::new("Arial", 11, [System.Drawing.FontStyle]::Bold)
$FontHeading2 = [System.Drawing.Font]::new("Arial", 14, [System.Drawing.FontStyle]::Bold)
$FontData     = [System.Drawing.Font]::new("Courier New", 10)

$DLL = '[DllImport("user32.dll")] public static extern bool ShowWindowAsync(IntPtr hWnd, int nCmdShow);'
Add-Type -MemberDefinition $DLL -name NativeMethods -namespace Win32

$Script:ToolName = "RemoveVMwUI"
$Script:IsAdmin            = [bool](([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator))
$Script:ScriptPath         = if (-not [string]::IsNullOrWhiteSpace($PSCommandPath)) { $PSCommandPath } else { $MyInvocation.MyCommand.Path }

# Logging Setup
$Script:ScriptName     = (Get-Item -Path $Script:ScriptPath).BaseName
$Script:LogDirectory  = Join-Path -Path $env:TEMP -ChildPath $Script:ScriptName
$Script:LogFile       = Join-Path -Path $Script:LogDirectory -ChildPath "$($Script:ScriptName)_$(Get-Date -Format 'yyyy-MM-dd_HH-mm-ss').log"
if (-not (Test-Path -Path $Script:LogDirectory -PathType Container)) {
    [void](New-Item -Path $Script:LogDirectory -ItemType Directory -Force -ErrorAction SilentlyContinue)
}

# Settings File Setup
$Script:SettingsDirectory = Join-Path -Path $env:LOCALAPPDATA -ChildPath "DeploymentBunny"
$Script:SettingsFile = Join-Path -Path $Script:SettingsDirectory -ChildPath "$($Script:ScriptName).settings.json"

# Generate logo
$PictureString = ""
$imageBytes = [Convert]::FromBase64String($PictureString)
$imageStream = New-Object System.IO.MemoryStream(,$imageBytes)
$LogoImage = [System.Drawing.Image]::FromStream($imageStream)

$consoleHandle = (Get-Process -Id $PID).MainWindowHandle
if ($consoleHandle -ne [IntPtr]::Zero) {
    [Win32.NativeMethods]::ShowWindowAsync($consoleHandle, 2)
}

# --- Form ---
$Form = New-Object System.Windows.Forms.Form
$Form.Text          = "RemoveVMwUI"
$Form.StartPosition = "CenterScreen"
$Form.Size          = New-Object System.Drawing.Size(1024, 700)
$Form.MinimumSize   = New-Object System.Drawing.Size(900, 580)
$Form.BackColor     = [System.Drawing.Color]::White

$LabelTitle          = New-Object System.Windows.Forms.Label
$LabelTitle.Location = New-Object System.Drawing.Point(16, 12)
$LabelTitle.Size     = New-Object System.Drawing.Size(750, 24)
$LabelTitle.Text     = "Completely remove virtual machines, for real"
$LabelTitle.Font     = $FontHeading2
$LabelTitle.BackColor = [System.Drawing.Color]::White
$Form.Controls.Add($LabelTitle)

$PictureBox          = New-Object System.Windows.Forms.PictureBox
$PictureBox.Location = New-Object System.Drawing.Point(842, 4)
$PictureBox.Size     = New-Object System.Drawing.Size(150, 70)
$PictureBox.Image    = $LogoImage
$PictureBox.SizeMode = [System.Windows.Forms.PictureBoxSizeMode]::Zoom
$PictureBox.BackColor = [System.Drawing.Color]::White
$PictureBox.Anchor   = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Right
$Form.Controls.Add($PictureBox)

$ButtonConnect          = New-Object System.Windows.Forms.Button
$ButtonConnect.Location = New-Object System.Drawing.Point(16, 46)
$ButtonConnect.Size     = New-Object System.Drawing.Size(170, 32)
$ButtonConnect.Text     = "Connect"
$ButtonConnect.Font     = $Font1
$ButtonConnect.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#F35800")
$ButtonConnect.ForeColor = [System.Drawing.ColorTranslator]::FromHtml("#1c1d1d")
$Form.Controls.Add($ButtonConnect)

$LabelStatus          = New-Object System.Windows.Forms.Label
$LabelStatus.Location = New-Object System.Drawing.Point(512, 52)
$LabelStatus.Size     = New-Object System.Drawing.Size(322, 20)
$LabelStatus.Text     = "Ready"
$LabelStatus.Font     = $FontHeading1
$LabelStatus.BackColor = [System.Drawing.Color]::White
$LabelStatus.Anchor   = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Left
$Form.Controls.Add($LabelStatus)

$LabelVMs          = New-Object System.Windows.Forms.Label
$LabelVMs.Location = New-Object System.Drawing.Point(16, 88)
$LabelVMs.Size     = New-Object System.Drawing.Size(976, 20)
$LabelVMs.Text     = "Select virtual machine(s):"
$LabelVMs.Font     = $FontHeading1
$LabelVMs.BackColor = [System.Drawing.Color]::White
$Form.Controls.Add($LabelVMs)

$GridVMs = New-Object System.Windows.Forms.DataGridView
$GridVMs.Location = New-Object System.Drawing.Point(16, 112)
$GridVMs.Size = New-Object System.Drawing.Size(976, 200)
$GridVMs.Font = $FontData
$GridVMs.ReadOnly = $true
$GridVMs.AllowUserToAddRows = $false
$GridVMs.AllowUserToDeleteRows = $false
$GridVMs.AllowUserToResizeRows = $false
$GridVMs.RowHeadersVisible = $false
$GridVMs.SelectionMode = [System.Windows.Forms.DataGridViewSelectionMode]::FullRowSelect
$GridVMs.MultiSelect = $true
$GridVMs.AutoSizeColumnsMode = [System.Windows.Forms.DataGridViewAutoSizeColumnsMode]::Fill
$GridVMs.Anchor = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Left -bor [System.Windows.Forms.AnchorStyles]::Right

[void]$GridVMs.Columns.Add('VMName', 'VM Name')
[void]$GridVMs.Columns.Add('State', 'State')
[void]$GridVMs.Columns.Add('UsesDifferencingDisk', 'Differencing Disk')
$GridVMs.Columns['VMName'].FillWeight = 56
$GridVMs.Columns['State'].FillWeight = 22
$GridVMs.Columns['UsesDifferencingDisk'].FillWeight = 22
$GridVMs.Columns['State'].DefaultCellStyle.Alignment = [System.Windows.Forms.DataGridViewContentAlignment]::MiddleCenter
$GridVMs.Columns['UsesDifferencingDisk'].DefaultCellStyle.Alignment = [System.Windows.Forms.DataGridViewContentAlignment]::MiddleCenter
$Form.Controls.Add($GridVMs)

$ButtonDelete          = New-Object System.Windows.Forms.Button
$ButtonDelete.Location = New-Object System.Drawing.Point(196, 46)
$ButtonDelete.Size     = New-Object System.Drawing.Size(170, 32)
$ButtonDelete.Text     = "Delete Selected VMs"
$ButtonDelete.Font     = $Font1
$ButtonDelete.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#D9534F")
$ButtonDelete.ForeColor = [System.Drawing.Color]::White
$Form.Controls.Add($ButtonDelete)

$CheckboxVerbose          = New-Object System.Windows.Forms.CheckBox
$CheckboxVerbose.Location = New-Object System.Drawing.Point(380, 50)
$CheckboxVerbose.Size     = New-Object System.Drawing.Size(120, 24)
$CheckboxVerbose.Text     = "Verbose"
$CheckboxVerbose.Font     = $Font1
$CheckboxVerbose.BackColor = [System.Drawing.Color]::White
$Form.Controls.Add($CheckboxVerbose)

$TextOutput             = New-Object System.Windows.Forms.TextBox
$TextOutput.Location    = New-Object System.Drawing.Point(16, 358)
$TextOutput.Size        = New-Object System.Drawing.Size(976, 250)
$TextOutput.Multiline   = $true
$TextOutput.ScrollBars  = "Both"
$TextOutput.ReadOnly    = $true
$TextOutput.WordWrap    = $false
$TextOutput.Font        = $FontData
$TextOutput.BackColor   = [System.Drawing.Color]::White
$TextOutput.Anchor      = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Bottom -bor [System.Windows.Forms.AnchorStyles]::Left -bor [System.Windows.Forms.AnchorStyles]::Right
$Form.Controls.Add($TextOutput)

$ButtonElevate          = New-Object System.Windows.Forms.Button
$ButtonElevate.Location = New-Object System.Drawing.Point(16, 616)
$ButtonElevate.Size     = New-Object System.Drawing.Size(170, 32)
$ButtonElevate.Text     = "Elevate (Admin)"
$ButtonElevate.Font     = $Font1
$ButtonElevate.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#FFC107")
$ButtonElevate.ForeColor = [System.Drawing.Color]::Black
$ButtonElevate.Anchor   = [System.Windows.Forms.AnchorStyles]::Bottom -bor [System.Windows.Forms.AnchorStyles]::Left
$ButtonElevate.Visible  = (-not $Script:IsAdmin)
$Form.Controls.Add($ButtonElevate)

$ButtonClose          = New-Object System.Windows.Forms.Button
$ButtonClose.Location = New-Object System.Drawing.Point(822, 616)
$ButtonClose.Size     = New-Object System.Drawing.Size(170, 32)
$ButtonClose.Text     = "Close"
$ButtonClose.Font     = $Font1
$ButtonClose.Anchor   = [System.Windows.Forms.AnchorStyles]::Bottom -bor [System.Windows.Forms.AnchorStyles]::Right
$Form.Controls.Add($ButtonClose)

# --- Events ---
$Form.Add_Load({
    $loadedSettings = Import-UISettings
    if ($loadedSettings) {
        if ($null -ne $loadedSettings.VerboseChecked) {
            $CheckboxVerbose.Checked = [bool]$loadedSettings.VerboseChecked
        }
    }
})

$Form.Add_FormClosing({
    Save-UISettings
})
$ButtonConnect.Add_Click({ Connect-TSxUI })
$ButtonClose.Add_Click({ Close-TSxUI })
$ButtonDelete.Add_Click({ Remove-TSxUI })
$CheckboxVerbose.Add_CheckedChanged({
    if ($CheckboxVerbose.Checked) {
        Add-OutputLine "Log file: $($Script:LogFile)"
    }
})
$ButtonElevate.Add_Click({
    try {
        if ([string]::IsNullOrWhiteSpace($Script:ScriptPath) -or -not (Test-Path -Path $Script:ScriptPath)) {
            throw "Unable to determine script path for elevation."
        }

        Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$Script:ScriptPath`"" -Verb RunAs -ErrorAction Stop
        $Form.Close()
    }
    catch {
        [void][System.Windows.Forms.MessageBox]::Show(
            $Form,
            ("Failed to relaunch elevated.`r`n{0}" -f $_.Exception.Message),
            $Script:ToolName,
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        )
    }
})

# --- Functions ---
function Save-UISettings {
    try {
        if (-not (Test-Path -LiteralPath $Script:SettingsDirectory -PathType Container)) {
            [void](New-Item -Path $Script:SettingsDirectory -ItemType Directory -Force -ErrorAction SilentlyContinue)
        }

        $settings = [PSCustomObject]@{
            VerboseChecked = $CheckboxVerbose.Checked
        }

        $settings | ConvertTo-Json -Depth 3 | Out-File -LiteralPath $Script:SettingsFile -Encoding UTF8
        Add-LogLine "Settings saved: $($Script:SettingsFile)"
    }
    catch {
        Add-LogLine ("Failed to save settings. Error: {0}" -f $_.Exception.Message)
    }
}

function Import-UISettings {
    if (-not (Test-Path -LiteralPath $Script:SettingsFile -PathType Leaf)) {
        return $null
    }

    try {
        $settings = Get-Content -LiteralPath $Script:SettingsFile -Raw | ConvertFrom-Json
        Add-LogLine "Settings loaded: $($Script:SettingsFile)"
        return $settings
    }
    catch {
        Add-LogLine ("Failed to load settings. Error: {0}" -f $_.Exception.Message)
        return $null
    }
}

function Add-OutputLine {
    param([string]$Text)
    $TextOutput.AppendText($Text + [Environment]::NewLine)
    $TextOutput.ScrollToCaret()
}

function Add-LogLine {
    param([string]$Text)
    $ts = "{0:yyyy-MM-dd HH:mm:ss.fff}" -f (Get-Date)
    $msg = "[$ts] $Text"
    try { Add-Content -Path $Script:LogFile -Value $msg -ErrorAction SilentlyContinue } catch {}
}

function Add-VerboseLine {
    param([string]$Text)

    if ($CheckboxVerbose.Checked) {
        Add-OutputLine ("[verbose] {0}" -f $Text)
    }
    Add-LogLine ("VERBOSE: $Text")
}

function Remove-TSxVM {
    [cmdletbinding(SupportsShouldProcess = $True)]
    Param(
        [parameter(Mandatory = $True, ValueFromPipelineByPropertyName = $true, Position = 0)]
        [ValidateNotNullOrEmpty()]
        $VMName
    )

    Add-VerboseLine ("Remove requested for VM '{0}'" -f $VMName)
    Add-LogLine "REMOVE: Starting removal of VM '$VMName'"

    $ScriptBlock = {
        param($TargetVMName)

        $Item = Get-VM -Name $TargetVMName -ErrorAction SilentlyContinue
        if ($null -eq $Item -or $Item.Count -eq 0) {
            return
        }

        Write-Output "Working on $TargetVMName"

        if ($Item.State -eq "Running") {
            Write-Output "Stopping $TargetVMName"
            Get-VM -Id $Item.Id | Stop-VM -Force -TurnOff
        }

        if ((Get-VM -Name $TargetVMName | Get-VMSnapshot).Count -ne 0) {
            Write-Output "$TargetVMName has snapshots, restoring and removing..."
            Get-VMSnapshot -VMName $TargetVMName | Where-Object ParentCheckpointName -EQ $null | Restore-VMSnapshot -Confirm:$false
            Remove-VMSnapshot -VMName $TargetVMName
            do { Start-Sleep -Seconds 1 } until ((Get-VM -Name $TargetVMName | Get-VMSnapshot).Count -eq 0)
        }
        else {
            Write-Output "$TargetVMName has no snapshots"
        }

        $Disks = Get-VMHardDiskDrive -VM $Item
        Write-Output ("Found {0} disk(s) attached" -f @($Disks).Count)
        foreach ($Disk in $Disks) {
            Write-Output "Removing disk: $($Disk.Path)"
            Remove-Item -Path $Disk.Path -Force -ErrorAction Continue
        }

        $ItemLoc = (Get-VM -Id $Item.Id).ConfigurationLocation
        Write-Output "Removing VM: $TargetVMName"
        Get-VM -Id $Item.Id | Remove-VM -Force
        Write-Output "Removing folder: $ItemLoc"
        Remove-Item -Path $ItemLoc -Recurse -Force
    }

    Add-VerboseLine "Executing remove operation locally"
    Add-LogLine "REMOVE: Executing locally"
    & $ScriptBlock $VMName
    Add-LogLine "REMOVE: Completed removal sequence for VM '$VMName'"
}

function Get-TSxVMInventory {
    Add-VerboseLine "Collecting VM inventory from local host"

    $scriptBlock = {
        Get-VM | Sort-Object Name | ForEach-Object {
            $vm = $_
            $usesDifferencingDisk = @(
                Get-VMHardDiskDrive -VMName $vm.Name -ErrorAction SilentlyContinue |
                    Where-Object { -not [string]::IsNullOrWhiteSpace($_.Path) } |
                    ForEach-Object {
                        $vhd = Get-VHD -Path $_.Path -ErrorAction SilentlyContinue
                        $null -ne $vhd -and $vhd.VhdType -eq "Differencing"
                    }
            ) -contains $true

            [pscustomobject]@{
                Name                 = $vm.Name
                State                = [string]$vm.State
                UsesDifferencingDisk = if ($usesDifferencingDisk) { "Yes" } else { "No" }
            }
        }
    }

    Add-VerboseLine "Inventory mode: local host"
    & $scriptBlock
}

function Connect-TSxUI {
    $LabelStatus.Text = "Connecting..."
    $Form.Refresh()
    try {
        Add-LogLine "UI: Connect sequence started for local host"
        Add-VerboseLine "Starting connect sequence"
        $VMs = @(Get-TSxVMInventory)
        $GridVMs.Rows.Clear()
        foreach ($VM in $VMs) {
            [void]$GridVMs.Rows.Add($VM.Name, $VM.State, $VM.UsesDifferencingDisk)
        }
        $runningCount = @($VMs | Where-Object State -eq 'Running').Count
        $diffCount = @($VMs | Where-Object UsesDifferencingDisk -eq 'Yes').Count
        Add-VerboseLine ("Inventory summary: Running={0}, DifferencingDisk={1}" -f $runningCount, $diffCount)
        Add-LogLine "UI: Successfully connected. Loaded $($VMs.Count) VMs (Running: $runningCount, Differencing: $diffCount)"
        Add-OutputLine "Connected to local host. Found $($VMs.Count) VM(s)."
        $LabelStatus.Text = "Connected - $($VMs.Count) VM(s) found"
    }
    catch {
        Add-LogLine "UI: Connect sequence FAILED - $($_.Exception.Message)"
        Add-VerboseLine "Connect sequence failed"
        Add-OutputLine "ERROR: Failed to connect to local host: $($_.Exception.Message)"
        $LabelStatus.Text = "Connection failed"
    }
}

function Close-TSxUI {
    $Form.Close()
}

function Remove-TSxUI {
    $SelectedItems = @($GridVMs.SelectedRows | ForEach-Object { [string]$_.Cells['VMName'].Value } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    Add-VerboseLine ("Selected VM count: {0}" -f $SelectedItems.Count)
    Add-LogLine "UI: Delete initiated - selected $($SelectedItems.Count) VM(s)"
    if ($SelectedItems.Count -eq 0) {
        Add-LogLine "UI: Delete cancelled - no VMs selected"
        [void][System.Windows.Forms.MessageBox]::Show(
            $Form,
            "Please select one or more virtual machines.",
            "Remove-VMUI",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Warning
        )
        return
    }

    $confirm = [System.Windows.Forms.MessageBox]::Show(
        $Form,
        "You are about to permanently delete $($SelectedItems.Count) VM(s) from the local host.`n`nThis cannot be undone. Continue?",
        "Remove-VMUI - Confirm Delete",
        [System.Windows.Forms.MessageBoxButtons]::YesNo,
        [System.Windows.Forms.MessageBoxIcon]::Warning
    )
    if ($confirm -ne [System.Windows.Forms.DialogResult]::Yes) {
        Add-LogLine "UI: Delete cancelled by user confirmation"
        return
    }

    $LabelStatus.Text = "Deleting $($SelectedItems.Count) VM(s)..."
    $Form.Refresh()
    Add-VerboseLine ("Delete order: {0}" -f ($SelectedItems -join ', '))
    Add-LogLine "UI: Delete confirmed - processing $($SelectedItems.Count) VM(s): $($SelectedItems -join ', ')"

    foreach ($SelectedItem in $SelectedItems) {
        Add-OutputLine "Deleting '$SelectedItem'..."
        Add-LogLine "OPERATION: Starting removal of VM '$SelectedItem'"
        try {
            $verboseLines = Remove-TSxVM -VMName $SelectedItem
            if ($CheckboxVerbose.Checked) {
                foreach ($line in $verboseLines) { Add-OutputLine "  $line" }
            }
            Add-OutputLine "Done: '$SelectedItem' removed."
            Add-LogLine "OPERATION: Successfully removed VM '$SelectedItem'"
        }
        catch {
            Add-VerboseLine ("Exception caught during deletion of '{0}'" -f $SelectedItem)
            Add-LogLine "OPERATION: FAILED to remove VM '$SelectedItem' - $($_.Exception.Message)"
            Add-OutputLine "ERROR removing '$SelectedItem': $($_.Exception.Message)"
        }
    }

    Add-OutputLine "--- Operation complete ---"
    Add-LogLine "UI: Delete operation completed"
    $LabelStatus.Text = "Delete complete"

    [void][System.Windows.Forms.MessageBox]::Show(
        $Form,
        "Done. Check the output pane for details.",
        "Remove-VMUI",
        [System.Windows.Forms.MessageBoxButtons]::OK,
        [System.Windows.Forms.MessageBoxIcon]::Information
    )
    Connect-TSxUI
}

[void]$Form.ShowDialog()

