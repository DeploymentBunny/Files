...
$InstallerFiles = Get-ChildItem -Path $SOURCEROOT -Filter *.msixbundle


foreach($InstallerFile in $InstallerFiles){
    Write-Output "$ScriptName - Add-AppxProvisionedPackage -PackagePath $($InstallerFile.fullname) -Regions All -Online -Verbose -SkipLicense"
    Add-AppxProvisionedPackage -PackagePath $($InstallerFile.fullname) -Regions All -Online -Verbose -SkipLicense
}
