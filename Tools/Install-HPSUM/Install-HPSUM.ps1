...
#Add SNMP
Add-WindowsFeature -Name 'SNMP-Service','SNMP-WMI-Provider' -IncludeAllSubFeature -IncludeManagementTools

#Copy folder
$Destination = "$env:TEMP\HPSP"
Copy-Item -Path "$SOURCEROOT" -Destination $Destination -Force -Recurse -Verbose

$ISO = Get-ChildItem $Destination -Filter *.iso
$ISOMount = Mount-DiskImage -ImagePath $ISO.FullName -PassThru | Get-Volume
$DriveLetter = "$($ISOMount.DriveLetter):"
$DriveLetter

#Set vars
$Arguments = "/c $DriveLetter\hp\swpackages\hpsum.bat /s"
$Executable = "cmd.exe"
