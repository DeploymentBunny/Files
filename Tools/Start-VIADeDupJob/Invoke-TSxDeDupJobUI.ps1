...
.FUNCTIONALITY
    ...
    The UI wraps Invoke-TSxDeDupJob.ps1 via a background PowerShell runspace and polls
    output every 400 ms via a WinForms Timer. Supports local/remote target selection,
    optional credential selection for remote remoting, Run and Show Running Jobs operations,
    local-only elevation enforcement at run time, verbose output toggle, persisted settings,
    and graceful cleanup on close.
#>


$Host.UI.RawUI.WindowTitle = $myInvocation.MyCommand.Definition

# Defensive cleanup: stop/dispose any OutputTimer left over from a previous run in this session
try { if ($null -ne $OutputTimer) { $OutputTimer.Stop(); $OutputTimer.Dispose() } } catch {}
...
$Script:ToolName = "Invoke-TSxDeDupJobUI"
$Script:TargetScriptPath = Join-Path $PSScriptRoot "Invoke-TSxDeDupJob.ps1"
...
$Script:SettingsFile = Join-Path $Script:SettingsDirectory "Invoke-TSxDeDupJobUI.settings.json"
$Script:Runner = $null
$Script:RunnerHandle = $null
$Script:RunnerOutput = $null
$Script:OutputIndex = 0
$Script:IsRunning = $false
$Script:CurrentOperation = "Run"
...
function Test-IsLocalTarget {
    ...
    $normalizedName = $ComputerName.Trim()
    if ([string]::IsNullOrWhiteSpace($normalizedName)) {
        return $true
    }

    if ($normalizedName -eq '.' -or
        $normalizedName -ieq 'localhost' -or
        $normalizedName -ieq $env:COMPUTERNAME) {
        return $true
    }

    if (-not [string]::IsNullOrWhiteSpace($env:USERDNSDOMAIN)) {
        $fqdn = "{0}.{1}" -f $env:COMPUTERNAME, $env:USERDNSDOMAIN
        if ($normalizedName -ieq $fqdn) {
            return $true
        }
    }
...
$Form.Text = "Invoke-TSxDeDupJobUI"
...
$LabelTitle.Text = "Run Data Deduplication jobs on all dedup-enabled volumes"
...
$ButtonRun.Location = New-Object System.Drawing.Point(16, 72)
$ButtonRun.Size = New-Object System.Drawing.Size(150, 32)
$ButtonRun.Text = "Run DeDup Jobs"
...
$ButtonReport.Location = New-Object System.Drawing.Point(176, 72)
$ButtonReport.Size = New-Object System.Drawing.Size(150, 32)
$ButtonReport.Text = "Show Running Jobs"
$ButtonReport.Font = $Font1
$ButtonReport.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#F5B041")
$ButtonReport.ForeColor = [System.Drawing.ColorTranslator]::FromHtml("#1c1d1d")
$Form.Controls.Add($ButtonReport)

$LabelServer = New-Object System.Windows.Forms.Label
$LabelServer.Location = New-Object System.Drawing.Point(16, 112)
$LabelServer.Size = New-Object System.Drawing.Size(60, 20)
$LabelServer.Text = "Server:"
$LabelServer.Font = $FontHeading1
...
$TextServer.Location = New-Object System.Drawing.Point(78, 108)
$TextServer.Size = New-Object System.Drawing.Size(260, 24)
...
$ButtonCredential.Location = New-Object System.Drawing.Point(348, 106)
$ButtonCredential.Size = New-Object System.Drawing.Size(120, 30)
...
$LabelCredential.Location = New-Object System.Drawing.Point(478, 111)
$LabelCredential.Size = New-Object System.Drawing.Size(514, 22)
...
$LabelCredential.Anchor = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Left -bor [System.Windows.Forms.AnchorStyles]::Right
$Form.Controls.Add($LabelCredential)

$ButtonClose = New-Object System.Windows.Forms.Button
$ButtonClose.Location = New-Object System.Drawing.Point(892, 616)
...
$ButtonElevate.Size = New-Object System.Drawing.Size(120, 32)
...
$CheckboxVerbose.Location = New-Object System.Drawing.Point(340, 75)
...
$LabelStatus.Location = New-Object System.Drawing.Point(500, 79)
$LabelStatus.Size = New-Object System.Drawing.Size(492, 20)
...
$TextOutput.Location = New-Object System.Drawing.Point(16, 142)
$TextOutput.Size = New-Object System.Drawing.Size(976, 466)
...
$OutputTimer.Add_Tick({
    ...
    if ($Script:RunnerHandle.IsCompleted) {
        try {
            ...
            Add-OutputLine -Text ("[{0}] {1} completed" -f (Get-Date -Format "HH:mm:ss"), $Script:CurrentOperation)
            Write-TSxLog -Message ("{0} completed" -f $Script:CurrentOperation)
        }
        catch {
            $LabelStatus.Text = "Failed"
            Add-OutputLine -Text ("ERROR: {0}" -f $_.Exception.Message)
            Write-TSxLog -Message ("{0} failed: {1}" -f $Script:CurrentOperation, $_.Exception.Message)
            Show-TSxDialog -Message ("Failed to run Invoke-TSxDeDupJob.ps1. {0}`r`n`r`nLog: {1}" -f $_.Exception.Message, $Script:LogFile) -Title $Script:ToolName -Icon Error -Owner $Form
        }
        finally {
            if ($Script:Runner) {
...
function Start-TSxDeDupOperation {
    param(
        [Parameter(Mandatory = $true)]
        [bool]$ReportMode,
    ...
    if (-not (Test-Path -LiteralPath $Script:TargetScriptPath -PathType Leaf)) {
        Show-TSxDialog -Message ("Could not find Invoke-TSxDeDupJob.ps1 at: {0}" -f $Script:TargetScriptPath) -Title $Script:ToolName -Icon Error -Owner $Form
        Write-TSxLog -Message ("Target script not found: {0}" -f $Script:TargetScriptPath)
        return
    }

    $targetServer = $TextServer.Text.Trim()
    if ([string]::IsNullOrWhiteSpace($targetServer)) {
        $targetServer = $env:COMPUTERNAME
    }

    $isLocalTarget = Test-IsLocalTarget -ComputerName $targetServer
    if ($isLocalTarget -and -not $Script:IsAdmin) {
        Show-TSxDialog -Message "Local execution requires elevation. Click 'Elevate (Admin)' and run again." -Title $Script:ToolName -Icon Warning -Owner $Form
        Write-TSxLog -Message ("{0} blocked because process is not elevated" -f $OperationLabel)
        return
    }

    $scriptContent = Get-Content -LiteralPath $Script:TargetScriptPath -Raw

    $Script:CurrentOperation = $OperationLabel
    $ButtonRun.Enabled = $false
    $ButtonReport.Enabled = $false
    $ButtonCredential.Enabled = $false
    $TextServer.ReadOnly = $true
    $Form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    $LabelStatus.Text = "{0} on {1}..." -f $OperationLabel, $targetServer
    $TextOutput.Clear()
    $showVerbose = $CheckboxVerbose.Checked
    $credentialUser = if ($Script:SelectedCredential) { $Script:SelectedCredential.UserName } else { "Current user" }

    Add-OutputLine -Text ("[{0}] Starting {1} on {2} (ShowVerbose={3}, Credential={4})" -f (Get-Date -Format "HH:mm:ss"), $OperationLabel, $targetServer, $showVerbose, $credentialUser)
    Write-TSxLog -Message ("{0} started. Server={1}; ReportMode={2}; ShowVerbose={3}; Credential={4}" -f $OperationLabel, $targetServer, $ReportMode, $showVerbose, $credentialUser)
    ...
    try {
        ...
        [void]$Script:Runner.AddScript({
            param($targetPath, $runVerbose, $targetServerName, $runLocal, [System.Management.Automation.PSCredential]$credentialObject, $rawScriptContent, $runReport)

            if ($runLocal) {
                if ($runReport) {
                    if ($runVerbose) {
                        & $targetPath -Report -Verbose *>&1
                    }
                    else {
                        & $targetPath -Report *>&1
                    }
            ...
            $remoteScriptBlock = {
                param($scriptText, $remoteVerbose, $remoteReport)

                $scriptToRun = [ScriptBlock]::Create($scriptText)
                if ($remoteReport) {
                    if ($remoteVerbose) {
                        & $scriptToRun -Report -Verbose *>&1
                    }
                    else {
                        & $scriptToRun -Report *>&1
                    }
                }
                else {
                    if ($remoteVerbose) {
                        & $scriptToRun -Verbose *>&1
                    }
                    else {
                        & $scriptToRun *>&1
                    }
            ...
            $invokeParams = @{
                ...
                ArgumentList = @($rawScriptContent, $runVerbose, $runReport)
                ErrorAction  = 'Stop'
            }
        ...
        [void]$Script:Runner.AddArgument($Script:TargetScriptPath)
        [void]$Script:Runner.AddArgument($showVerbose)
        [void]$Script:Runner.AddArgument($targetServer)
        [void]$Script:Runner.AddArgument($isLocalTarget)
        [void]$Script:Runner.AddArgument($Script:SelectedCredential)
        [void]$Script:Runner.AddArgument($scriptContent)
        [void]$Script:Runner.AddArgument($ReportMode)
        ...
        $LabelStatus.Text = "Failed"
        Add-OutputLine -Text ("ERROR: {0}" -f $_.Exception.Message)
        Write-TSxLog -Message ("{0} startup failed: {1}" -f $OperationLabel, $_.Exception.Message)
        Show-TSxDialog -Message ("Failed to start Invoke-TSxDeDupJob.ps1. {0}`r`n`r`nLog: {1}" -f $_.Exception.Message, $Script:LogFile) -Title $Script:ToolName -Icon Error -Owner $Form
        $Form.Cursor = [System.Windows.Forms.Cursors]::Default
        $ButtonRun.Enabled = $true
        $ButtonReport.Enabled = $true
        $ButtonCredential.Enabled = $true
...
$ButtonCredential.Add_Click({
    $selectedCredential = $Host.UI.PromptForCredential(
        "Remote Credential",
        "Enter credential for remote PowerShell connection.",
        "",
        ""
    )

    if ($null -ne $selectedCredential) {
        $Script:SelectedCredential = $selectedCredential
        $LabelCredential.Text = "Credential: {0}" -f $selectedCredential.UserName
        Write-TSxLog -Message ("Credential selected for remote execution. User: {0}" -f $selectedCredential.UserName)
    }
})
...
$Form.Add_FormClosing({
    ...
    # Always stop and dispose the output timer so it cannot fire in subsequent runs
    try { $OutputTimer.Stop(); $OutputTimer.Dispose() } catch {}
...
$ButtonRun.Add_Click({
    Start-TSxDeDupOperation -ReportMode $false -OperationLabel "Run"
})

$ButtonReport.Add_Click({
    Start-TSxDeDupOperation -ReportMode $true -OperationLabel "Report"
})
