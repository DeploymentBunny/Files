...
$Installers = Get-ChildItem -Path $SOURCEROOT -Filter *.exe -Recurse

foreach($Installer in $Installers){
    $Args = "/sALL /rs /MSI /qn /norestart ALLUSERS=1 EULA_ACCEPT=YES DISABLE_ARM_SERVICE=1 UPDATE_MODE=0"
    $Exec = """$($Installer.FullName)"""
    Invoke-VIAExe -Executable $Exec -Arguments $Args -Verbose
}
