...
.DESCRIPTION
    Remove-VMUI2 provides a Windows Forms interface for Hyper-V VM cleanup.
    - Starts without requiring elevation at launch.
    ...
    - Connects to a local or remote Hyper-V host, lists available VMs, and permanently
      deletes selected VMs including checkpoints/snapshots, attached virtual disk
      files, and the configuration folder.
    - Supports alternate credentials for remote host operations.
    - Verbose mode can be toggled via checkbox; log filename is shown in the output pane.
    - Displays operation output in a scrollable output pane.
    - Confirms destructive deletions before proceeding.
    - Writes a timestamped log file to %TEMP%\Remove-VMUI2\.
    - Persists last-used settings (hostname, Verbose state) to
      %LOCALAPPDATA%\DeploymentBunny\Remove-VMUI2.settings.json.
...
.FUNCTIONALITY
    Starts directly in Windows Forms UI without prompting at startup.
    If running as a standard user, an "Elevate (Admin)" button is shown in the form.
    Clicking it relaunches the script elevated via Start-Process -Verb RunAs and closes
    the current non-elevated UI instance.
    Requires local Administrator privileges to manage local Hyper-V VMs.
    On form load, last-used hostname and Verbose checkbox state are restored from the
    settings file. On form close, current values are saved back.
#>
...
$LabelTitle.Text     = "Completely remove virtual machines, for real"
...
$LabelServer.Location = New-Object System.Drawing.Point(16, 90)
$LabelServer.Size     = New-Object System.Drawing.Size(110, 20)
$LabelServer.Text     = "Hyper-V Host:"
...
$TextServer.Location = New-Object System.Drawing.Point(130, 86)
$TextServer.Size     = New-Object System.Drawing.Size(160, 24)
...
$LabelStatus.Location = New-Object System.Drawing.Point(512, 52)
$LabelStatus.Size     = New-Object System.Drawing.Size(322, 20)
$LabelStatus.Text     = "Ready"
$LabelStatus.Font     = $FontHeading1
$LabelStatus.BackColor = [System.Drawing.Color]::White
$LabelStatus.Anchor   = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Left
$Form.Controls.Add($LabelStatus)

$LabelVMs          = New-Object System.Windows.Forms.Label
$LabelVMs.Location = New-Object System.Drawing.Point(16, 118)
$LabelVMs.Size     = New-Object System.Drawing.Size(976, 20)
...
$GridVMs.Location = New-Object System.Drawing.Point(16, 142)
$GridVMs.Size = New-Object System.Drawing.Size(976, 200)
...
$GridVMs.Anchor = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Left -bor [System.Windows.Forms.AnchorStyles]::Right

[void]$GridVMs.Columns.Add('VMName', 'VM Name')
[void]$GridVMs.Columns.Add('State', 'State')
[void]$GridVMs.Columns.Add('UsesDifferencingDisk', 'Differencing Disk')
$GridVMs.Columns['VMName'].FillWeight = 56
$GridVMs.Columns['State'].FillWeight = 22
$GridVMs.Columns['UsesDifferencingDisk'].FillWeight = 22
$GridVMs.Columns['State'].DefaultCellStyle.Alignment = [System.Windows.Forms.DataGridViewContentAlignment]::MiddleCenter
$GridVMs.Columns['UsesDifferencingDisk'].DefaultCellStyle.Alignment = [System.Windows.Forms.DataGridViewContentAlignment]::MiddleCenter
$Form.Controls.Add($GridVMs)

$ButtonDelete          = New-Object System.Windows.Forms.Button
$ButtonDelete.Location = New-Object System.Drawing.Point(196, 46)
$ButtonDelete.Size     = New-Object System.Drawing.Size(170, 32)
$ButtonDelete.Text     = "Delete Selected VMs"
$ButtonDelete.Font     = $Font1
$ButtonDelete.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#D9534F")
...
$CheckboxVerbose.Location = New-Object System.Drawing.Point(380, 50)
...
$ButtonCredential.Location = New-Object System.Drawing.Point(300, 86)
...
$LabelCredential.Location = New-Object System.Drawing.Point(480, 90)
$LabelCredential.Size     = New-Object System.Drawing.Size(460, 20)
...
$TextOutput.Location    = New-Object System.Drawing.Point(16, 358)
$TextOutput.Size        = New-Object System.Drawing.Size(976, 250)
...
$Form.Add_Load({
    ...
    if ($loadedSettings) {
        if (-not [string]::IsNullOrWhiteSpace($loadedSettings.HostName)) {
            $TextServer.Text = [string]$loadedSettings.HostName
        }
...
$ButtonDelete.Add_Click({ Remove-TSxUI })
$CheckboxVerbose.Add_CheckedChanged({
    if ($CheckboxVerbose.Checked) {
        Add-OutputLine "Log file: $($Script:LogFile)"
    }
})
$ButtonCredential.Add_Click({ Set-TSxCredential })
$ButtonElevate.Add_Click({
    try {
        if ([string]::IsNullOrWhiteSpace($Script:ScriptPath) -or -not (Test-Path -Path $Script:ScriptPath)) {
...
function Save-UISettings {
    try {
        ...
        Add-LogLine "Settings saved: $($Script:SettingsFile)"
    }
    catch {
        Add-LogLine ("Failed to save settings. Error: {0}" -f $_.Exception.Message)
    }
}
...
function Import-UISettings {
    ...
    try {
        ...
        Add-LogLine "Settings loaded: $($Script:SettingsFile)"
        return $settings
    }
    catch {
        Add-LogLine ("Failed to load settings. Error: {0}" -f $_.Exception.Message)
        return $null
    }
}
...
function Set-TSxCredential {
    try {
        ...
        [void][System.Windows.Forms.MessageBox]::Show($Form, ("Failed to select credential.`r`n{0}" -f $_.Exception.Message), $Script:ToolName, [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
}
...
function Add-LogLine {
    param([string]$Text)
    $ts = "{0:yyyy-MM-dd HH:mm:ss.fff}" -f (Get-Date)
    $msg = "[$ts] $Text"
    try { Add-Content -Path $Script:LogFile -Value $msg -ErrorAction SilentlyContinue } catch {}
}

function Test-TSxIsLocalComputer {
    ...
    try {
        $localHostEntry = [System.Net.Dns]::GetHostEntry($env:COMPUTERNAME)
        if (-not [string]::IsNullOrWhiteSpace($localHostEntry.HostName)) {
            $localComputerNames += $localHostEntry.HostName.ToLowerInvariant()
        }
    }
    catch {
    }

    return $normalizedComputername -in $localComputerNames
}
...
function Remove-TSxVM {
    ...
    $ScriptBlock = {
        ...
        $Item = Get-VM -Name $TargetVMName -ErrorAction SilentlyContinue
        if ($null -eq $Item -or $Item.Count -eq 0) {
            return
        }

        Write-Output "Working on $TargetVMName"

        if ($Item.State -eq "Running") {
            Write-Output "Stopping $TargetVMName"
            Get-VM -Id $Item.Id | Stop-VM -Force -TurnOff
        }

        if ((Get-VM -Name $TargetVMName | Get-VMSnapshot).Count -ne 0) {
            Write-Output "$TargetVMName has snapshots, restoring and removing..."
            Get-VMSnapshot -VMName $TargetVMName | Where-Object ParentCheckpointName -EQ $null | Restore-VMSnapshot -Confirm:$false
            Remove-VMSnapshot -VMName $TargetVMName
            do { Start-Sleep -Seconds 1 } until ((Get-VM -Name $TargetVMName | Get-VMSnapshot).Count -eq 0)
        }

        $Disks = Get-VMHardDiskDrive -VM $Item
        foreach ($Disk in $Disks) {
            Write-Output "Removing disk: $($Disk.Path)"
            Remove-Item -Path $Disk.Path -Force -ErrorAction Continue
        }

        $ItemLoc = (Get-VM -Id $Item.Id).ConfigurationLocation
        Write-Output "Removing VM: $TargetVMName"
        Get-VM -Id $Item.Id | Remove-VM -Force
        Write-Output "Removing folder: $ItemLoc"
        Remove-Item -Path $ItemLoc -Recurse -Force
    }

    if (Test-TSxIsLocalComputer -Computername $Computername) {
        & $ScriptBlock $VMName
    }
    else {
        Invoke-Command -ComputerName $Computername -ScriptBlock $ScriptBlock -ArgumentList $VMName
    }
}

function Get-TSxVMInventory {
    param(
    ...
    $scriptBlock = {
        Get-VM | Sort-Object Name | ForEach-Object {
            ...
            $usesDifferencingDisk = @(
                Get-VMHardDiskDrive -VMName $vm.Name -ErrorAction SilentlyContinue |
                    ...
                    ForEach-Object {
                        ...
                        $null -ne $vhd -and $vhd.VhdType -eq "Differencing"
                    }
            ) -contains $true
            ...
            [pscustomobject]@{
                ...
                UsesDifferencingDisk = if ($usesDifferencingDisk) { "Yes" } else { "No" }
            }
        }
    }
    ...
    elseif ($null -ne $Script:SelectedCredential) {
        Invoke-Command -ComputerName $Computername -Credential $Script:SelectedCredential -ScriptBlock $scriptBlock
    }
    else {
...
function Connect-TSxUI {
    $LabelStatus.Text = "Connecting to $($TextServer.Text)..."
    $Form.Refresh()
    try {
        Add-LogLine "UI: Connect sequence started for host '$($TextServer.Text)'"
        $VMs = @(Get-TSxVMInventory -Computername $TextServer.Text)
        $GridVMs.Rows.Clear()
        foreach ($VM in $VMs) {
            [void]$GridVMs.Rows.Add($VM.Name, $VM.State, $VM.UsesDifferencingDisk)
        }
        Add-LogLine "UI: Successfully connected. Loaded $($VMs.Count) VMs"
        Add-OutputLine "Connected to $($TextServer.Text). Found $($VMs.Count) VM(s)."
        $LabelStatus.Text = "Connected - $($VMs.Count) VM(s) found"
    }
    catch {
        Add-LogLine "UI: Connect sequence FAILED - $($_.Exception.Message)"
        Add-OutputLine "ERROR: Failed to connect to $($TextServer.Text): $($_.Exception.Message)"
...
function Remove-TSxUI {
    ...
    Add-LogLine "UI: Delete initiated - selected $($SelectedItems.Count) VM(s) from host '$($TextServer.Text)'"
    if ($SelectedItems.Count -eq 0) {
        Add-LogLine "UI: Delete cancelled - no VMs selected"
        [void][System.Windows.Forms.MessageBox]::Show(
            $Form,
    ...
    $confirm = [System.Windows.Forms.MessageBox]::Show(
        $Form,
        "You are about to permanently delete $($SelectedItems.Count) VM(s) from '$($TextServer.Text)'.`n`nThis cannot be undone. Continue?",
        "RemoveVMwUI2 - Confirm Delete",
        [System.Windows.Forms.MessageBoxButtons]::YesNo,
        [System.Windows.Forms.MessageBoxIcon]::Warning
    )
    if ($confirm -ne [System.Windows.Forms.DialogResult]::Yes) {
        Add-LogLine "UI: Delete cancelled by user confirmation"
        return
    }

    $LabelStatus.Text = "Deleting $($SelectedItems.Count) VM(s)..."
    $Form.Refresh()
    Add-LogLine "UI: Delete confirmed - processing $($SelectedItems.Count) VM(s): $($SelectedItems -join ', ')"

    foreach ($SelectedItem in $SelectedItems) {
        Add-OutputLine "Deleting '$SelectedItem' from $($TextServer.Text)..."
        Add-LogLine "OPERATION: Starting removal of VM '$SelectedItem'"
        try {
            $verboseLines = Remove-TSxVM -Computername $($TextServer.Text) -VMName $SelectedItem
            if ($CheckboxVerbose.Checked) {
                foreach ($line in $verboseLines) { Add-OutputLine "  $line" }
            }
            Add-OutputLine "Done: '$SelectedItem' removed."
            Add-LogLine "OPERATION: Successfully removed VM '$SelectedItem'"
        }
        catch {
            Add-LogLine "OPERATION: FAILED to remove VM '$SelectedItem' - $($_.Exception.Message)"
            Add-OutputLine "ERROR removing '$SelectedItem': $($_.Exception.Message)"
        }
    }

    Add-OutputLine "--- Operation complete ---"
    Add-LogLine "UI: Delete operation completed"
    $LabelStatus.Text = "Delete complete"

    [void][System.Windows.Forms.MessageBox]::Show(
        $Form,
        "Done. Check the output pane for details.",
        "RemoveVMwUI2",
        [System.Windows.Forms.MessageBoxButtons]::OK,
